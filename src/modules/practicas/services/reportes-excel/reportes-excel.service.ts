...
export class ReportesExcelService {
  ...
  async generarReporteSemestral(tipo_practica: TipoPractica, estado_aprobacion: Estado_informe) {
    const reporteAlumnos = new ExcelJS.Workbook();


    if(estado_aprobacion == Estado_informe.DESAPROBADA){
      const worksheet = reporteAlumnos.addWorksheet('ListaDesaprobadosICISemestral')
      worksheet.columns = [
        { header: 'nombreAlumno', key:'nombreAlumno' },
        { header: 'rutAlumno', key: 'rutAlumno' },
        { }
      ]
    }else {
      const worksheet = reporteAlumnos.addWorksheet('ListaAprobadosICISemestral')
    }
